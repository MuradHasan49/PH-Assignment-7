...
import { BiSolidMessageDetail, BiChevronDown, BiSearch } from 'react-icons/bi'
...
const TimeLinePage = () => {
  ...
  const filterOptions = [
    { id: "all", label: "All Interactions", icon: <IoGrid />, data: interactionsData, show: true, bg: "bg-gray-100", text: "text-gray-600" },
    { id: "call", label: "Calls only", icon: <IoCall />, data: Calls, show: Calls.length > 0, bg: "bg-blue-50", text: "text-blue-600" },
    { id: "text", label: "Text only", icon: <BiSolidMessageDetail />, data: Texts, show: Texts.length > 0, bg: "bg-green-50", text: "text-green-600" },
    { id: "video", label: "Video only", icon: <FaRegFileVideo />, data: Videos, show: Videos.length > 0, bg: "bg-purple-50", text: "text-purple-600" },
  ]

  const currentOption = filterOptions.find(opt => opt.label === selectedLabel)

  useEffect(() => {
    const baseData = currentOption ? currentOption.data : interactionsData;
    const filtered = baseData.filter(item =>
      item.user.name.toLowerCase().includes(searchQuery.toLowerCase()) ||
      item.action.toLowerCase().includes(searchQuery.toLowerCase())
    );
    setdata(filtered);
  }, [searchQuery, selectedLabel, interactionsData]);
  ...
  return (
    <div className="container mx-auto p-4 md:p-8 font-sans bg-[#f8fafc] min-h-screen">
      ...
      <div className="flex flex-col md:flex-row gap-4 mb-10 items-center justify-between">
        ...
        <div className="relative w-full md:w-80 group">
          <div className="absolute inset-y-0 left-4 flex items-center pointer-events-none">
            <BiSearch className="text-gray-400 text-xl group-focus-within:text-[#244D3F] transition-colors" />
          </div>
          <input
            type="text"
            ...
            className="w-full pl-12 pr-5 py-3.5 bg-white border border-gray-100 rounded-2xl shadow-sm outline-none transition-all duration-300 hover:border-[#244D3F] focus:border-[#244D3F] focus:ring-4 focus:ring-[#244D3F]/5 text-sm font-medium text-gray-700"
          />
        </div>
      </div>
      ...
      <div className="space-y-4">
        {data.length > 0 ? (
          data.map((item, ind) => (
            <div key={ind} className="bg-white border border-gray-100 rounded-2xl p-5 flex items-center gap-5 shadow-[0_2px_10px_rgba(0,0,0,0.02)] hover:shadow-md transition-all duration-300 animate-in fade-in slide-in-from-bottom-2">
          ...
          <div className="flex flex-col items-center justify-center py-20 bg-white rounded-3xl border border-dashed border-gray-200">
            <p className="text-gray-400 font-medium">No interactions match your search.</p>
          </div>
        )}
      </div>
    </div>
